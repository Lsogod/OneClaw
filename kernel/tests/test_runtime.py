...
class KernelRuntimeTests(unittest.TestCase):
    ...
    def test_tool_registry_context_and_compact_policy_are_exposed(self) -> None:
        with tempfile.TemporaryDirectory() as root:
            ...
            Path(root, ".oneclaw").mkdir(parents=True, exist_ok=True)
            Path(root, ".oneclaw", "issue.md").write_text("# Issue\n\nFix Windows CI.\n", "utf-8")
            Path(root, ".oneclaw", "pr_comments.md").write_text("# PR Comments\n\n- app.ts:1: simplify branch.\n", "utf-8")
            ...
            self.assertIn("Fix Windows CI", system_prompt)
            ...
            self.assertEqual(context["projectInstructions"]["count"], 3)
            kernel.shutdown()
