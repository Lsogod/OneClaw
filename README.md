...
/swarm status <name>
/swarm artifact <name>
/swarm review <name> <status> [note]
/swarm merge <name> <status> [note]
```

`/swarm review`、`/swarm merge` 和 `/swarm artifact` 会把当前 swarm 状态写入本地 artifact catalog，方便后续审计、导出或合并总结。
...
| `/provider setup-plan` | 输出 provider wizard 风格的配置和修复步骤 |
