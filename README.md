...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Tests-109_TS_%2B_26_Python-brightgreen" alt="Tests">
...
- MCP status/tools/resources/templates 浏览、resource detail、template 填参读取、server reconnect
- artifact viewer，用于查看 tool result、swarm summary、session export、diagnostic bundle
...
`/swarm split` 会用当前 provider 把 swarm goal 拆成可执行 subtasks。`/swarm review <name> auto` 会运行 review prompt 并生成 review artifact；`/swarm merge <name> summary` 会运行 merge summary prompt 并生成 merge artifact；`/swarm diff` 会汇总关联 worktree 或当前 workspace 的 git diff/stat。`/swarm review`、`/swarm merge`、`/swarm diff` 和 `/swarm artifact` 都会把结果写入本地 artifact catalog，方便后续审计、导出或合并总结。
...
```text
/channels list [query]
/channels add <kind> <name> [--label <label>] [--secret-env <ENV>] [--webhook-path <path>]
/channels send <name> <message>
...
`POST /channels/:name/messages` 可以只记录 inbound message，也可以带 `run: true` 直接启动一个 bridge-managed background task，把 OneClaw 从本地/HTTP bridge 推向多入口 agent gateway。
...
外部 adapter 通过 stdin 接收 JSON payload，并在 stdout 返回 JSON result。未配置或非 strict 失败时会自动回退到内置 Python scanner。
...
/plugin marketplace install <name> [--dry-run|--trust]
/plugin reload
/hooks files
...
- MCP dynamic add/remove/reconnect/read/templates/auth/browser
- channels/gateway registry，支持 inbox、ack 和 bridge-managed task 触发
- plugin lifecycle、marketplace、audit、trust、remote source lockfile
...
- memory/session 管理
- task/team/swarm 基础生命周期，包含 split/review-auto/merge-summary/diff artifacts
- sandbox fallback smoke
- macOS/Linux/Windows CI 全绿

后续主要是生态厚度工作：更多 tool、更多 provider 真实 E2E、MCP auth/resource 复杂场景、plugin 远程来源策略强化，以及更完整的 vim/voice 子系统。
